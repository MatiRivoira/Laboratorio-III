...
app.get("/productos_db", (request, response) => {
    request.getConnection((error, conn) => {
        if (error) {
            throw ("Ha ocurrido un error al conectarse a la base de datos");
        }
        conn.query("SELECT * FROM productos", (error, resultado) => {
            if (error) {
                throw ("Error al intentar hacer la consulta");
            }
...
//CRUD BASE DE DATOS
//##############################################################################################//
app.post('/productos_db', upload.single("foto"), (request, response) => {
    ...
    request.getConnection((error, conexion) => {
        ...
        conexion.query("INSERT INTO productos SET ?", [obj], (error, resultado) => {
            if (error) {
                throw ("Ocurrio un error al generar la consulta");
            }
            response.send("Producto agregado correctamente");
        });
    });
});
app.put('/productos_db', upload.single("foto"), (request, response) => {
    let file = request.file;
    let extension = mime.extension(file.mimetype);
    let obj = JSON.parse(request.body.obj);
    let path = file.destination + obj.codigo + "." + extension;
    ...
    request.getConnection((error, conexion) => {
        ...
        conexion.query("UPDATE productos SET ? WHERE codigo=?", [obj_modif, obj.codigo], (error, resultado) => {
            if (error) {
                throw ("Ocurrio un error al generar la consulta");
            }
...
app.delete('/productos_db', (request, response) => {
    ...
    request.getConnection((error, conexion) => {
        ...
        conexion.query("SELECT path FROM productos WHERE codigo=?", [obj.codigo], (error, resultado) => {
            if (error) {
                throw ("Ocurrio un error al generar la consulta");
            }
            path_foto += resultado[0].path;
        });
    });
    request.getConnection((error, conexion) => {
        if (error) {
        ...
        conexion.query("DELETE FROM productos WHERE codigo=?", [obj.codigo], (error, resultado) => {
            if (error) {
                throw ("Ocurrio un error al generar la consulta");
            }
            fs.unlink(path_foto, (error) => {
                if (error) {
