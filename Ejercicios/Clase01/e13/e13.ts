...
function descomponerEnFactoresPrimos(numero: number): number[] {
    const factoresPrimos: number[] = [];
    let divisor = 2;
  ...
  function sumarCoeficientes(factoresPrimos: number[]): number {
  ...
  function esNumeroDeSmith(numero: number): boolean {
    if (numero < 4) {
