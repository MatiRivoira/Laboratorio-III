...
class Empleado extends Usuario implements ICRUD
{
    ...
    public static function TraerTodos(): array{
        try {
            ...
            $sql = $pdo->prepare("SELECT empleados.id, empleados.nombre, empleados.correo, empleados.clave, empleados.id_perfil, empleados.foto, empleados.sueldo, perfiles.descripcion AS descripcion FROM empleados
            ...
            if ($sql->execute()) {
                ...
                if ($empleados !== false) {
                    foreach ($empleados as $empleado) {
                        $retorno[] = new Empleado($empleado["id"], $empleado["nombre"], $empleado["correo"], $empleado["clave"], $empleado["id_perfil"], $empleado["descripcion"], Empleado::ConstruirFoto($empleado["foto"]), $empleado["sueldo"]);
                    }
                }
                return $retorno;
            }
    ...
    public static function ConstruirFoto(string $path) : array {
        $nombreArchivo = basename($path); // Obtiene el nombre del archivo de la URL
        return array(
            "name" => $nombreArchivo,
            "type" => "", // Determina el tipo MIME del archivo
            "size" => 0,
            "tmp_name" => $path, // Aquí podrías usar la URL, pero es incorrecto
            "error" => UPLOAD_ERR_OK // Suponiendo que no hubo errores
        );
