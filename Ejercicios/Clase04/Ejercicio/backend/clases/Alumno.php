...
class Alumno 
{
    ...
    static function ListarObjetos() {
        ...
        if ($archivo) {
            while (!feof($archivo)) {
                $linea = fgets($archivo);
                $elementosFile = explode("-", $linea); //separo la linea por su -
                $legajo = trim($elementosFile[0]); // saco los espacios en el elemento donde se encuentra el legajo
                if ($legajo != "") {
                    $nombre = trim($elementosFile[1]);
                    $apellido = trim($elementosFile[2]);
                    $foto = trim($elementosFile[3]);
                    $alumnos[] = new Alumno($legajo, $nombre, $apellido, $foto);
                }
            }
            fclose($archivo);
        }
    ...
    static function ListarTabla() : string {
        $html = '<table>';
        $html .= '<thead>
                    <th>Legajo</th>
                    <th>Nombre</th>
